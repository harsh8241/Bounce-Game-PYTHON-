...
root.resizable(0, 0)
root.wm_attributes("-topmost", 1) #to take the root window above all windows
canvas = Canvas(root, width=500, height=500, bd=0, highlightthickness=0, highlightbackground="Red", bg="Black") #highlightthickness to remove the border
canvas.pack(padx=10, pady=10)
score = Label(height=50, width=80, text="Score: 00", font="Consolas 14 bold")
score.pack(side="left")
root.update()#update elements of the gui
...
class Ball:
    def __init__(self, canvas, color, paddle, bricks, score):
        ...
        self.hit = 0
        self.id = canvas.create_oval(10, 10, 25, 25, fill=color, width=1)#Creates a circle or an ellipse at the given coordinates. It takes two pairs of coordinates; the top left and bottom right corners of the bounding rectangle for the oval.
        self.canvas.move(self.id, 230, 461)#to initiate the movement from current position
        start = [4, 3.8, 3.6, 3.4, 3.2, 3, 2.8, 2.6]
        random.shuffle(start)#to rearrange start
        #print(start)
        self.x = start[0]
        self.y = -start[0]
                ...
                try:
                    if pos[2] >= brick_pos[0] and pos[0] <= brick_pos[2]:
                        if pos[3] >= brick_pos[1] and pos[1] <= brick_pos[3]:
                            canvas.bell()
                            self.hit += 1
                            self.score.configure(text="Score: " + str(self.hit))#updating the score
                            self.canvas.delete(brick.id)#deleting the brick when it has been hit
                            return True
                except:
                    continue
        return False
...
class Paddle:
    ...
    def draw(self):#drawing the paddle
        pos = self.canvas.coords(self.id)
        #print(pos)
        if pos[0] + self.x <= 0:
            self.x = 0
...
def start_game(event):
    ...
    if playing is False:
        ...
        while 1:
            if paddle.pausec !=1:
                try:
                    canvas.delete(m)#m is the text for pause
                    del m
                except:
                    pass
                if not ball.bottom_hit:
                    ...
                    root.update_idletasks()
                    root.update()
                    time.sleep(0.01)#for time delay of 1/100 of a sec
                    if ball.hit==95:
...
j=canvas.find_all()
root.mainloop() #to run our application,This method will loop forever until user exits
